...
export function getFunctions(input: string): FunctionDescription[] {
	...
	for (const [name, parts] of Object.entries(intermediateResult)) {
		for (const singleInside of parts.inside) {
			...
			result = [...result, ...getFunctions(singleInside)];
		}
	}
	...
	return result
		.filter(maybeNewElement => {
			const isContained = alreadyContained.some(alreadyContainedElement =>
				_.isEqual(maybeNewElement, alreadyContainedElement),
			);
			if (!isContained) alreadyContained.push(maybeNewElement);

			return !isContained;
		})
		.sort((element, otherElement) => {
			if (element.name !== otherElement.name) return element.name.localeCompare(otherElement.name);
...
export function getTopLevelFunctionSections(input: string): FunctionParts {
	...
	for (const nextCharacter of input) {
		...
		if (nextCharacter === ')') {
			...
			if (level === 0) {
				if (result[currentFunctionName]) result[currentFunctionName].inside.push(currentInside);
				else result[currentFunctionName] = { inside: [currentInside] };
