...
fun validateFunctionAndConstructorDefinitionsAreUnique(request: ProveStatementRequest) {
    ...
    if(set.size != request.functionDefinitions.size + constructorFunctions.size)
        throw SemanticException("Function and/or constructor definitions are duplicated! (There may only be one function of each name)")
}
...
fun validateFunctionsWithConstructorInputAreFlat(definition: DefinitionWithDetails) {
    ...
    if(definition.details.inputTypes.size != 1)
        throw SemanticException("The defined function ${definition.details.name}/${definition.details.arity} must have arity 1, because it uses an input constructor")
...
fun validateArityOfInputTypesAndInputVariablesIsEqual(definition: DefinitionWithDetails) {
    ...
    if(definition.inputVariables.size != definition.details.inputTypes.size)
        throw SemanticException("The defined function ${definition.details.name}/${definition.details.arity} has a different arity than it's input variables (arity ${definition.inputVariables.size})")
}
    ...
    request.functionDefinitions.forEach { functionDefinition ->
        functionDefinition.definition.forEachIndexed { index, definition ->
            if (definition.inputConstructor != null && definition.inputVariable != null)
               throw SemanticException("The ${index + 1}. definition for ${functionDefinition.name}/${functionDefinition.arity} contains both input_constructor and input_variables. It may use at most one.")
        }
    }
}